...
| **生命周期管理** | `touch`/`free`/`evict` 等方法管理 block 从分配到释放的全生命周期 |
...
`blocks: list[KVCacheBlock]` 是按 `block_id` 索引的全部逻辑块数组，启动时一次性创建 `KVCacheBlock(i) for i in range(num_blocks)`。

`KVCacheBlock`（`kv_cache_utils.py:117-178`）是 BlockPool 管理的基本单元——一个"轻量元数据壳"，只承载四类元数据，**不持有任何 torch.Tensor / 显存指针**：
...
> 实际可分配块数 = `num_blocks - 1`（减去 null_block）。
...
```python
# block_pool.py:198-221
def get_cached_block(
    self,
    block_hash: BlockHash,               # 组无关的内容哈希
    group_ids: tuple[int, ...],           # Full Attention单group: (0,)
) -> KVCacheBlock | None:
```

**作用**：给定一个内容哈希和 group 列表，返回可复用的缓存块；任一 group miss 即返回 `None`。

Full Attention 单 group 下，`group_ids=(0,)`，实际就是查 `cached_block_hash_to_block[(block_hash, 0)]`。命中返回 block，未命中返回 None。

### 5.2 分配新块 `get_new_blocks`

```python
# block_pool.py:223-248
def get_new_blocks(self, num_blocks: int) -> list[KVCacheBlock]:
    blocks = self.free_block_queue.popleft_n(num_blocks)  # 从队首弹出
    for block in blocks:
        assert block.ref_cnt == 0
        if self.enable_caching:
            self._maybe_evict_cached_block(block)  # 清掉旧缓存条目
        block.ref_cnt = 1
    self.metrics_collector.on_blocks_allocated(num_blocks)
    return blocks
```

**流程**：
1. 从空闲链表**队首**弹出 n 个块（最该被驱逐的优先分配）
2. 对每个块：若启用前缀缓存且该块仍挂着旧哈希，先 `_remove_cached_block_hashes` 清理（避免旧 hash 错指向新内容）
3. `ref_cnt = 1`，返回给上层
...
- `enable_caching=False` 时统一走 `with_hash` 侧按 recency 追加

**为什么要求 `ordered_blocks` 是逆序？** 调用方（`free` 方法）传入的 block 列表是按从尾到头的顺序释放的（先释放最新尾部块，再释放老块），使得尾部 block 先入空闲队列、老的前缀 block 后入队列。这保证了 LRU 顺序的正确性。

### 5.5 缓存满块 `cache_full_blocks`

```python
# block_pool.py:326-373
def cache_full_blocks(
    self,
    blocks: list[KVCacheBlock],
    block_hashes: list[BlockHash],
    num_tokens: int | None = None,
    extra_keys: list[tuple[BlockHash, int]] | None = None,
) -> None:
```

**作用**：当 block 被 token 填满后（`block_size` 个 token 全部写入），计算链式哈希并存入映射表，使其成为可被前缀缓存命中的条目。

流程（对每个 block）：
1. 若块已有主哈希（partial→full 晋升场景），先清掉旧别名
2. `_insert_block_hash` 写入新主哈希
3. 有事件时发 `BlockStored` 事件

### 5.6 驱逐缓存条目 `evict_blocks`

```python
# block_pool.py:295-310
def evict_blocks(self, block_ids: list[int]) -> None:
```

**作用**：仅按 `block_id` 从哈希表移除缓存条目，**不改变块的占用状态**。`ref_cnt > 0` 的块仍被请求持有，只是失去"可被前缀命中"的身份。常用于显式缓存失效。

### 5.7 哈希转移 `move_block_hashes`

```python
# block_pool.py:375-400
...
`hash_block_tokens()`（`kv_cache_utils.py:596`）实现链式哈希：
...
`NONE_HASH` 是链头种子，默认 `os.urandom(32)` 防碰撞。

---

## 七、端到端生命周期示例

以 Llama 模型（`block_size=16`，10个block，id 0~9）为例，演示两个请求的完整生命周期。请求 A 有 40 个 token（3个满块），请求 B 与请求 A 前 16 个 token 相同。

```
【初始状态】BlockPool 初始化后
  null_block = block_0（is_null=True）
  FreeQueue: [1, 2, 3, 4, 5, 6, 7, 8, 9]（队首→队尾）
  HashMap:  {}

【步骤1：请求A到达，分配3个块】
  get_new_blocks(3) → popleft_n(3) → [block_1, block_2, block_3]
  → 每个块 ref_cnt=1，清除旧哈希（初始无哈希，无操作）
  A.req_to_blocks = [block_1, block_2, block_3]
  FreeQueue: [4, 5, 6, 7, 8, 9]

【步骤2：请求A完成前向，前32个token写满block_1和block_2】
  cache_full_blocks([block_1, block_2], [H0, H1])
  → block_1.set_block_hash((H0,0)), block_2.set_block_hash((H1,0))
  → HashMap: {(H0,0)→block_1, (H1,0)→block_2}
  FreeQueue: [4, 5, 6, 7, 8, 9]（不变，ref_cnt>0）

【步骤3：请求B到达，前缀查找】
  B的block_hashes = [H0, H1', H2']（H1'与H1不同，从第2块开始分叉）
  get_cached_block(H0, (0,)) → 命中 block_1
  get_cached_block(H1', (0,)) → 未命中 → break
  → 返回 [block_1]，即前16个token命中

  分配：touch([block_1]) 把block_1从空闲队列摘出（本来ref_cnt=1，不在队列中），ref_cnt→2
  get_new_blocks(2) → popleft_n(2) → [block_4, block_5]
  B.req_to_blocks = [block_1, block_4, block_5]
  → block_1 被A和B共享（零拷贝）！

【步骤4：请求A完成，逆序释放】
  free_blocks([block_3, block_2, block_1])
  → block_3: ref_cnt 1→0, 无hash → prepend（队首）
  → block_2: ref_cnt 1→0, 有hash(H1) → append（队尾）
  → block_1: ref_cnt 2→1（B还在用）→ 不进队列
  FreeQueue: [3, 4, 5, 6, 7, 8, 9, 2]
  HashMap:  {(H0,0)→block_1, (H1,0)→block_2}（不变）

【步骤5：请求B完成，逆序释放】
  free_blocks([block_5, block_4, block_1])
  → block_5: ref_cnt 1→0, 无hash → prepend
  → block_4: ref_cnt 1→0, 有hash(H2') → append
  → block_1: ref_cnt 1→0, 有hash(H0) → append
  FreeQueue: [5, 3, 6, 7, 8, 9, 2, 4, 1]
              ↑无哈希优先驱逐        ↑有哈希尽量保留
  HashMap:  {(H0,0)→block_1, (H1,0)→block_2, (H2',0)→block_4}
```

**要点串联**：
- 首次分配：`popleft_n` 从队首取块，填满后 `cache_full_blocks` 写入哈希
- 前缀命中：`get_cached_block` 查哈希表找到 block_1，`touch` 将 `ref_cnt++` 实现零拷贝共享
- 双队列分流释放：无哈希块（block_3、block_5）插队首，有哈希块（block_2、block_4、block_1）插队尾
...
- 一次 `popleft_n(k)` 拿到 `[n, n+1, ..., n+k-1]`，所有 group 用同一组编号，原子分配无需跨组同步
- 跨组命中对齐简单：命中第 k 个 block 时所有 group 都用 `block_id=k`

### E2. 跨组同时命中语义

`get_cached_block(hash, group_ids)` 输入一个组无关的 `BlockHash` + 一组 `group_ids`，对每个 group 用 `(hash, group_id)` 查哈希表；**任一 group miss 就整体返回 None**。

原因：一条请求需要所有 group 同时持有该 block 才能复用。如果 Full 组命中但 Mamba 组没命中，Mamba 仍需重新计算状态，整体上等于没命中。

### E3. 三种 block_size 关系

混合模型里不同 group 可能有不同物理 `block_size`（如 Full `block_size=16`、Mamba `block_size=32`）。`resolve_kv_cache_block_sizes()`（`kv_cache_utils.py:626-688`）通过 LCM/GCD 统一：
...
`BlockHashListWithBlockSize`（`kv_cache_utils.py:2224`）利用链式哈希"子哈希覆盖整个前缀"的特性，把 GCD 粒度的细哈希懒加载转换为各组目标粒度的哈希。

### E4. Partial block 缓存

当 `hash_block_size < block_size` 时，一个物理大块内部可能有多个 hash 边界。`cache_partial_block` 把大块在内部前缀边界注册为可命中的别名，此时 `_block_hash_num_tokens < block_size`。纯 Full Attention 单 group 场景下三种 block_size 相等，partial block 仅在 CoW 等特殊场景出现。
