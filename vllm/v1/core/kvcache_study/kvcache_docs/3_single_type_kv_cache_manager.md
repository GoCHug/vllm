...
> 时序位置：[`0_runtime_sequence.md`](./0_runtime_sequence.md) ③ 前缀查找 → ④ 分配与缓存 → ⑧ 释放
>
> 源文件：`vllm/vllm/v1/core/single_type_kv_cache_manager.py`
>
> 主线：纯 Full Attention 单 group，核心是子类 `FullAttentionManager`。**统一锚点：模型 Llama-3-8B（pp2tp2，4 卡，`block_size=16`）上的示例请求 R（prompt=70 token，前 32 token 为共享前缀 SP，由前置请求 P 缓存为块 1/2），详见时序文档 §2。本文重点：端到端时序路径上被 Coordinator 直接下放的 7 个方法，按请求经过顺序逐行看源码（短注释），且每个方法都带"示例 R 中发生了什么"参考块；其余辅助方法一张表带过。**
...
对应 [`0_runtime_sequence.md`](./0_runtime_sequence.md) 的端到端流程，`FullAttentionManager` 承担：

| 端到端阶段 | 职责 | 对应方法 | 示例 R 中发生了什么 |
|---|---|---|---|
| **③ 前缀查找** | 在 `cached_block_hash_to_block` 查最长已计算前缀 | `find_longest_cache_hit`（classmethod） | 查到 P 缓存的 SP 块 1/2，hit_length=32 |
| **④ 分配与缓存 · 算新块数** | 总 token 数 − 已命中块数 → 需新分配块数 | `get_num_blocks_to_allocate` | 70 token → 共 5 块 − 已命中 2 → 申请 3 块 |
| **④ 分配与缓存 · touch 命中块** | 命中块 `ref_cnt+=1`、移出 free 队列，防驱逐 | `add_local_computed_blocks` | touch 块 1/2，ref_cnt 0→1 |
| **④ 分配与缓存 · 外部命中** | 为远端/CPU 命中的 token 分配新物理块 | `allocate_external_computed_blocks` | 无 connector，恒不触发 |
| **④ 分配与缓存 · 分配新块** | 从 free 队列取块 | `allocate_new_blocks` | 弹出块 3/4/5 |
| **④ 分配与缓存 · 写缓存** | 满块哈希写入前缀表 | `cache_blocks` → `block_pool.cache_full_blocks` | 满块 3/4 入哈希表；尾块 5 未满不入 |
| **⑧ 释放** | 逆序遍历块 `ref_cnt-=1`，0 则回收 | `free` / `pop_blocks_for_free` | 逆序归还块 7…3；命中块 1/2 仅减计数 |
...
**示例 R**：首次 prefill 调本方法，`new_computed_blocks` = ③ 查到的命中块 1/2 → touch 后两块 `ref_cnt` 0→1，`req_to_blocks[R] = [block1, block2]`，`num_cached_block[R] = 2`。
...
**示例 R**：为 prefill 后 38 token 补块——`cdiv(70, 16) − len(req_blocks) = 5 − 2 = 3` → 调 `get_new_blocks(3)` 弹块 3/4/5，并记入 `new_block_ids` 供 Worker 清零。
...
**示例 R**：`num_tokens = 70` → `num_full_blocks = 4`，而 `num_cached_block[R] = 2`（两块为命中）→ 只缓存切片 `[2:4]` = 满块 3/4 入哈希表；尾块 5 未满不入。
...
**示例 R**：请求结束先 `pop_blocks_for_free` 弹出 `req_blocks = [1,2,3,4,5,6,7]` 并清 `num_cached_block[R]`；再 `free` 逆序归还 7→6→5→4→3；命中块 1/2 由 `free_blocks` 仅对 R 的引用 `ref_cnt--`（P 的缓存 SP 保留，供后续请求复用）。
...
**示例 R**：`block_hashes = [h(t0-15), h(t16-31), h(t32-47), h(t48-63)]`，`max_length = 69`。Phase 1 从头连续命中前 2 个满块（P 缓存的 SP）→ `computed_blocks = ([block1, block2],)`、`hit_length = 32`；第 3 个 hash（追加问题首块）miss → 链式即 break，后续无需再查。
