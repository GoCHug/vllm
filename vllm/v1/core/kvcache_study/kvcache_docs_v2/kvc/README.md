...
kvc/
├── README.md                          <- 本文件（目录总览）
├── llama.log                           端到端轮全量日志（773 行 = 启动 + P + R）
│
├── docs/                               【文档】
...
│   ├── README.md                       补丁讲解：为什么这么加、逐 patch 详解、应用/回滚、端到端实测记录
...
│
├── startup/                            【启动期产物】
│   └── kvc_startup.log                 KVCache 初始化全流程（158 行 = CFG82 + L1 68 + L2/L4/L5 5 + 3 原生行）
│
├── p/                                   【P 全套：缓冲 2 块】
...
（3）读轨迹：`grep '\[KVC\]' llama.log`（素引行见 `p/p_run_start.txt`、`r5/r_run_start.txt`）

完整命令、验证点与注意事项见 `docs/2` 与 `docs/3`。

## 4. 环境快照与关键实测数字

| 项 | 值 |
|---|---|
| Pod / 镜像 | gggtest (a3，4={"Ascend910"})；`antsys/vllm:v0.23.0-a3-openeuler-20260818163431_aarch64` |
...
| 物理张量 | K/V 分离：K_cache=V_cache=(13296, 128, 4, 128) bf16（每层 2M 对齐 int8 双池 1661.88MiB × 2） |
