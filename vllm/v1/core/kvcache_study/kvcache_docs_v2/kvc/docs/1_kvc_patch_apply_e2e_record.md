...
> 本文是 `2_kvc_cn_curl_case.md`（用例）与 `patch/`（打印补丁）的**端到端正式验证记录**：从干净源码出发，以 patch 方式注入 40 处 `[KVC]` 打印，记录一次完整的服务启动初始化与 P/R 双请求生命周期。实测时间 2026-09-23 10:38~10:39（容器时钟）。
>
...
## 3. 启动期 KVCache 初始化全流程（log/kvc_startup.log，155 行）

启动命令与就绪标志：

```bash
bash scripts/start.sh        # 服务后台启动, 日志 -> log/llama.log; 就绪标志: grep 'Application startup complete' log/llama.log
head -375 log/llama.log | grep '\[KVC\]' > log/kvc_startup.log    # 启动段 155 行纯 [KVC]
```
...
主线：① 算规格 → ② 测预算 → ③ `KVCacheSpec → KVCacheGroupSpec → KVCacheTensor → KVCacheConfig` → min 对齐下发。本轮 profile 实测 13296 块（`determine_available_memory` 跨次启动存在 ±1~2 块的测量波动）。

### 3.2 物理侧（4 个 worker 进程，68 行 L1，vllm-ascend 路径）

```
[Worker][model_runner_v1.py:4257] [KVC][L1] _allocate_kv_cache_tensors[dense]: model.layers.0.self_attn.attn: KVCacheTensor(size=3324.00MiB) -> K int8 1662.00MiB + V int8 1662.00MiB (alignment=2097152, device=npu:0~3)   # ×16 层/worker
...
INFO [block_pool.py:209] [KVC][L2] BlockPool.__init__: num_gpu_blocks=13296, KVCacheBlock × 13296, cached_block_hash_to_block=BlockHashToBlockMap(size=0), null_block=(0, is_null=True)
...
(EngineCore)  INFO [kv_cache_utils.py:1777] Maximum concurrency for 8,192 tokens per request: 207.75x
...
请求体 `log/req_p.json` / `log/req_r5.json` 由 `scripts/gen_cn_requests.py --gen` 生成（与 `2_kvc_cn_curl_case.md` §3 字节级一致）：

```
bash scripts/curl_p_r.sh     # 依次发送 P、R；自动记录起始行/打屏/响应, 并提取三条 [KVC] 轨迹到 log/
```

### 4.1 P：缓冲 2 块（log/kvc_p.log，33 行）
...
INFO [block_pool.py:84] [KVC][L2] get_one_block: key=(hash=db0caac46067, group_id=0) -> MISS
INFO [single_type_kv_cache_manager.py:598] [KVC][L3]   第 1 块 MISS: BlockHash=db0caac46067 -> break                        # 冷缓存, 首块断链
INFO [kv_cache_coordinator.py:494] [KVC][L4] find_longest_cache_hit 返回: hit_blocks=[[]], hit_length=0
INFO [kv_cache_manager.py:431] [KVC][L5] S1 get_num_blocks_to_allocate: 需分配 3 块 vs 可用 13295 块
INFO [block_pool.py:411] [KVC][L2] BlockPool.get_new_blocks(3): popleft_n -> block_ids=[1, 2, 3], 剩余 num_free_blocks=13292
INFO [block_pool.py:108] [KVC][L2] insert: (hash=db0caac46067) <- KVCacheBlock(block_id=1), map size=1    # 满块1 入表
INFO [block_pool.py:108] [KVC][L2] insert: (hash=9e0e5bc08064) <- KVCacheBlock(block_id=2), map size=2    # 满块2 入表
...
### 4.2 R：五块生命周期（log/kvc_r5.log，355 行）

```
① 复用 2 块 (第 3 hash MISS 断链):
INFO [request.py:185] [KVC][ENQ] Request(...) 入队: num_prompt_tokens=486, max_tokens=35, 满块链式哈希 × 3: ['db0caac46067', '9e0e5bc08064', 'c2ea7819f805']
INFO [kv_cache_coordinator.py:476] [KVC][L4] find_longest_cache_hit: 满块hash数=3, max_cache_hit_length=485
INFO [block_pool.py:70] [KVC][L2] get_one_block: (hash=db0caac46067) -> HIT KVCacheBlock(block_id=1)
INFO [single_type_kv_cache_manager.py:590] [KVC][L3]   第 1 块 HIT: BlockHash=db0caac46067 -> cached blocks=[1]
INFO [block_pool.py:70] [KVC][L2] get_one_block: (hash=9e0e5bc08064) -> HIT KVCacheBlock(block_id=2)
INFO [single_type_kv_cache_manager.py:590] [KVC][L3]   第 2 块 HIT: BlockHash=9e0e5bc08064 -> cached blocks=[2]
INFO [block_pool.py:84] [KVC][L2] get_one_block: (hash=c2ea7819f805) -> MISS
...
INFO [kv_cache_manager.py:431] [KVC][L5] S1 get_num_blocks_to_allocate: 需分配 4 块 vs 可用 13295 块            # S1 报总需求 cdiv(486,128)=4(含待 touch 2)
...
INFO [block_pool.py:108] [KVC][L2] insert: (hash=c2ea7819f805) <- KVCacheBlock(block_id=4), map size=3   # 块4 追问句恰填满
...
INFO [block_pool.py:108] [KVC][L2] insert: (hash=2e6bd855659a) <- KVCacheBlock(block_id=5), map size=4   # 刚满的块5 与跨界申请合并发生在步 27
...
|---|---|
| `log/llama.log`（772 行） | 本轮服务全量日志（启动 + 双请求） |
| `log/kvc_startup.log`（155 行） | 启动期 [KVC] 拆解轨迹（原生关键行见 §3.4） |
| `log/kvc_p.log`（33 行）/ `log/kvc_r5.log`（355 行） | P / R 运行期 [KVC] 拆解轨迹 |
...
| `log/p_run_start.txt` / `log/r_run_start.txt` | 双请求在 log/llama.log 中的起始行 |
